        ...
        if msg.content.lower() == 'confirm':
            ...
            if response[0].emoji == '✅':
                ...
                req_money = discord.Embed(
                    title='Send funds!',
                    description=f'{buyer.mention} Please send **__exactly__** **${rest}** + $3 (miners fee) **(${int(rest)+3})** or **{(1/btc_price) * (rest+3)}** BTC to **{self.key.address}**\n'\
                    ...
                    if len(transactions["pending"]) > 0:
                        ...
                        for i in transactions["pending"]:
                            if i["value"] < 0:
                                ...
                                if value / 100_000_000 <= (1/btc_price) * (rest+3) <= (1/btc_price * ((rest+3) + 10)):
                ...
                money_out = self.key.create_transaction(output, fee=30)
                await channel.send(f'Thank you for using middler bot, {to_send} has been sent to {addy} view it here : https://www.blockchain.com/btc/tx/{money_out}')
